<template>
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-5xl mx-auto px-4">
      <div class="mb-6">
        <router-link to="/github" class="text-blue-500 hover:underline text-xs"
          >← Volver a repositorios</router-link
        >
        <h1 class="text-2xl font-bold text-gray-900 mt-2">
          Estadísticas de {{ repo?.name || '...' }}
        </h1>
        <p class="text-gray-500 text-sm" v-if="repo?.description">{{ repo.description }}</p>
      </div>
        ...
        <RepoStatsGroup>
          <template #main>
            <RepoStatItem
              ...
              color="blue"
              class="bg-transparent border-0 shadow-none border-b-2 border-blue-200"
            />
            <RepoStatItem
              label="Forks"
              :value="repo.forks_count"
              color="purple"
              class="bg-transparent border-0 shadow-none border-b-2 border-purple-200"
            />
            <RepoStatItem
              label="Watchers"
              :value="repo.watchers_count"
              color="green"
              class="bg-transparent border-0 shadow-none border-b-2 border-green-200"
            />
            <RepoStatItem
              label="Lenguaje"
              :value="repo.language || '-'"
              color="gray"
              class="bg-transparent border-0 shadow-none border-b-2 border-gray-300"
            />
          </template>
          <template #totals>
            <RepoStatItem
